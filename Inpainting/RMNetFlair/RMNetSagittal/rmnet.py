...
from tensorflow.keras.layers import Input, Dense, Flatten, Dropout, Concatenate, Multiply, Lambda, Add
...
class RMNETWGAN():
    def __init__(self,config):
        ...
        #Configure Loader
        self.img_dir = r'/home/ntustison/Data/Inpainting/FLAIR/AlignedToNKISlices/'
        self.masks_dir = r'/home/ntustison/Data/Inpainting/R-MNet-Inpainting-keras/masks/train/qd_imd/train/'
        self.imgs_in_path = glob.glob(self.img_dir + "Images/sagittal/*.nii.gz")
...
#            self.combined.compile(loss=[self.generator_loss, self.wasserstein_loss],loss_weights=[1, 1e-3], optimizer=self.g_optimizer)
    ...
    def get_batch(self, imgs_index, batch_imgs):
        ...
        for i in range(batch_imgs):
            print("\rLoading image number "+ str(i) + " of " + str(batch_imgs) + " of " + str(len(self.imgs_in_path)), end = " ")
            ...
            real_img = ants.image_read(self.imgs_in_path[imgs_index]).numpy()
#            real_img = ants.resample_image(real_img, (self.img_width, self.img_height), use_voxels=True)
            real_img = real_img / (0.5 * real_img.max()) - 1.
            real_img = np.reshape(real_img, (self.img_width, self.img_height, 1))
            real_img = np.concatenate((real_img, real_img, real_img), axis=-1)
...
#            cv2.waitKey(0 )
    ...
    def train(self):
        ...
        #     self.generator.load_weights(initial_weights)
            ...
            for databatchs in range(dataLoads):
                ...
                for batch in range(batches):
                    ...
                    g_loss = self.combined.train_on_batch([imgs[idx_batches], masks[idx_batches]], 
                                                          [tf.concat([imgs[idx_batches], masks[idx_batches]], axis=-1),valid])

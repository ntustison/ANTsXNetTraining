...
inpainting_unet = antspynet.create_partial_convolution_unet_model_3d((*patch_size, number_of_channels),
                                                                     number_of_priors=0,
                                                                     number_of_filters=(32, 64, 128, 256, 256), #  number_of_filters=(32, 64, 128, 256, 512, 512),
                                                                     kernel_size=3)
...
def loss_total(x_mask):
    ...
    def l1_norm(y_true, y_pred):
        if len(y_true.shape) == 5:
            return K.mean(K.abs(y_pred - y_true), axis=[1,2,3,4])
    ...
    def gram_matrix(x):
        # Assertions on input
        assert K.ndim(x) == 5, 'Input tensor should be a 5d (B, H, W, D, C) tensor'
        ...
        gram = gram /  K.cast(C * H * W * D, x.dtype)
        return gram
    ...
    def loss_tv(mask, y_comp):
        ...
        dilated_mask = K.conv3d(K.cast(1-mask, 'float32'), kernel, data_format='channels_last', padding='same')
    ...
    def loss(y_true, y_pred):
        ...
        lsum = tf.math.reduce_mean(l1 + 6*l2 + 0.1*l6)
...
inpainting_weights_filename = scripts_directory + "flair_inpainting_3d_weights.h5"

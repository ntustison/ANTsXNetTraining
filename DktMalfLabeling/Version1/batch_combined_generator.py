...
def batch_generator(batch_size,
    ...
    while True:
        ...
        while batch_count < batch_size:
            ...
            segmentation[segmentation < 1000] = 0
            # Some subjects have the following extra labels
            segmentation[segmentation == 1032] = 0
            segmentation[segmentation == 1033] = 0
            segmentation[segmentation == 2032] = 0
            segmentation[segmentation == 2033] = 0

            left_labels = (1002, 1003, *tuple(range(1005, 1032)), 1034, 1035)
            right_labels = (2002, 2003, *tuple(range(2005, 2032)), 2034, 2035)
            for ll in range(len(right_labels)):
                segmentation[segmentation == right_labels[ll]] = left_labels[ll]
            ...
            if do_add_noise and random.sample((False, False, False, True), 1)[0]:
                tic = time.perf_counter()
        ...
        if verbose:
            ...
            if use_multiple_outputs:
                Y2 = np.expand_dims(np.sum(encoded_Y[:,:,:,:,7:], axis=4), axis=-1)

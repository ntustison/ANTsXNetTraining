...
{                                     
  ...
  if( is.null( spatialPriors ) )
    {
    stop( "Spatial priors are not specified." )
    }	     
  if( is.null( images ) )
    {
  ...
  function()
    ...
    X <- array( data = 0, dim = c( batchSize, imageSize, 1 + length( spatialPriors ) ) ) 
    for( i in seq.int( batchSize ) )
      {   
      for( p in seq.int( length( spatialPriors ) ) )
        {
        X[i,,,,p+1] <- as.array( spatialPriors[[p]] )
        }
      }
   Y <- array( data = 0L, dim = c( batchSize, imageSize ) )
    ...
    while( i <= batchSize )
      ...
      warpedImage <- resampleImage( image, imageSize, useVoxels = TRUE, interpType = 0 )
      warpedMask <- resampleImage( segmentationImage, imageSize, useVoxels = TRUE, interpType = 1 )
      ...
      if( doDataAugmentation == TRUE )
        ...
        simulatedMaskArray <- as.array( simulatedMask )

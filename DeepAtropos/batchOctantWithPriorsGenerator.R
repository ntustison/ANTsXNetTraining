...
{                                     
  ...
  priorsPatches <- list()
  for( i in seq.int( length( priors ) ) )
    {
    priorsPatches[[i]] <- extractImagePatches( priors[[i]], patchSize, maxNumberOfPatches = "all",
  ...
  function()
    ...
    X <- array( data = 0, dim = c( batchSize, patchSize, 8 ) )    
	...
	for( p in seq.int( length( priors ) ) )
	  {
	  X[i,,,,1+p] <- priorsPatches[[p]][i%%8+1,,,]
	  }

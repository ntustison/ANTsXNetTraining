...
def batch_generator(batch_size=32,
    ...
    while True:
        ...
        template_mask = ants.threshold_image(labels[0], 0, 0, 0, 1)
        ...
        while batch_count < batch_size:
            ...
            image = images[i]
            label = labels[i]
            image_mask = ants.threshold_image(label, 0, 0, 0, 1)
            ...
            if do_histogram_intensity_warping and random.sample((True, True, False), 1)[0]:
                ...
                for b in range(len(break_points)):
                    displacements.append(abs(random.gauss(0, 0.25)))
            ...
            if do_add_noise and random.sample((True, False), 1)[0]:
                ...
                noise_parameters = (0.1, random.uniform(0.15, 0.25))
            ...
            if do_random_transformation:
                ...
                image_mask = ants.threshold_image(data_augmentation['simulated_images'][0][1], 0.5, 1.1, 1, 0)
                label = data_augmentation['simulated_segmentation_images'][0]
                toc = time.perf_counter()
                # print(f"Xfrms {toc - tic:0.4f} seconds")

            if not resample_direction == "none":

...
baseDirectory <- '/home/ntustison/Data/BrainExtractionBold/'
...
brainWeightsFileName <- paste0( scriptsDirectory, "/brainExtractionBoldWeights.h5" )
...
brainBaseDataDirectory <- '/home/ntustison/Data/BrainExtractionBold/Data/'
brainImages <- list.files( path = brainBaseDataDirectory, pattern = "*average.nii.gz", recursive = TRUE, full.names = TRUE )
...
for( i in seq_len( length( brainImages ) ) )
  ...
  image <- brainImages[i]
  mask <- gsub( "average", "average-mask", image )

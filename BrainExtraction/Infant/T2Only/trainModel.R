...
scriptsDirectory <- paste0( baseDirectory, 'ScriptsT2Only/' )
...
brainWeightsFileName <- paste0( scriptsDirectory, "/brainExtractionInfant", imageModalities[1], ".h5" )
...
for( i in seq_len( length( maskImages ) ) )
  ...
  if( ! file.exists( t2 ) || ! file.exists( mask ) )
    {
    next  
